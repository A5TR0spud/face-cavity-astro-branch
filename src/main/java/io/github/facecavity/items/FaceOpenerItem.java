...
public class FaceOpenerItem extends Item {

    public FaceOpenerItem() {
    ...
    public boolean openChestCavity(PlayerEntity player, LivingEntity target, boolean shouldKnockback){
        ...
        if(optional.isPresent()){
            ...
            if(target == player || cc.getChestCavityType().isOpenable(cc)) {
                ...
                if(player.world.isClient) {
                    if (!target.getEquippedStack(EquipmentSlot.HEAD).isEmpty()) {
                        player.sendMessage(Text.literal("Target's face is obstructed"),true);
